  ...
  <div class="bg-white dark:bg-gray-800 transition-colors duration-200">
    ...
    <form @submit.prevent="salvar" class="p-6">
      <div class="space-y-6">
        ...
        <!-- Horário -->
        <div>
          <label for="horario" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Horário</label>
          <input
            type="time"
            id="horario"
        ...
        <div class="flex items-center gap-3">
          <button 
            ...
            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-gray-200 dark:focus:ring-gray-600 focus:ring-offset-2"
            :class="form.ativa ? 'bg-gray-900 dark:bg-indigo-600' : 'bg-gray-200 dark:bg-gray-600'"
          >
            <span
        ...
        <div>
          <div class="flex items-center justify-between mb-3">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Tarefas da rotina</label>
            <button 
              type="button" 
          ...
          <div v-if="form.tarefas.length === 0" class="text-sm text-gray-400 dark:text-gray-500 text-center py-8 border-2 border-dashed border-gray-100 dark:border-gray-700 rounded-lg">
            Nenhuma tarefa adicionada
          </div>
          
          <div v-else class="space-y-2">
            <div v-for="(tarefa, index) in form.tarefas" :key="index" class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <input
