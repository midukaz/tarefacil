    ...
    <form @submit.prevent="salvar" class="p-6">
      <div class="space-y-6">
        ...
        <div>
          ...
          <div class="flex gap-2">
            <input
              ...
              class="flex-1 px-4 py-3 bg-gray-50 border-0 rounded-lg focus:ring-2 focus:ring-gray-200 focus:bg-white transition-all duration-200 text-sm"
            />
            <button
              type="button"
              @click="adicionarTag"
              class="px-4 py-3 bg-gray-100 hover:bg-gray-200 rounded-lg text-gray-600 transition-colors text-sm font-medium"
            >
              Adicionar
            </button>
          </div>
          ...
          <div v-if="form.tags.length > 0" class="mt-3 flex flex-wrap gap-2">
            <span
              ...
              :key="tag"
              class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-gray-100 text-gray-700"
            >
              {{ tag }}
              <button
                type="button"
        ...
        <div>
          ...
          <div v-if="form.subtarefas.length === 0" class="text-sm text-gray-400 text-center py-8 border-2 border-dashed border-gray-100 rounded-lg">
            Nenhuma subtarefa adicionada
          </div>
          
          <div v-else class="space-y-2">
            <div v-for="(subtarefa, index) in form.subtarefas" :key="subtarefa.id || index" class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
              <input
                type="checkbox"
